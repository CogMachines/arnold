...
namespace VW
{
    ...
    void
    vw_main(const std::string& inArgs)
    {
        try {
            
            int argc = 0;
            char** argv = VW::get_argv_from_string(inArgs, argc);
            ...
            ftime(&t_end);
            const auto net_time = (int) (1000.0 * (t_end.time - t_start.time) + (t_end.millitm - t_start.millitm));
